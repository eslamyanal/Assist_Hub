...
public class DeleteRequest {
    ...
    private static DeleteRequest Retrofit = null ;
    private HTTP_Method HTTP_Method ;

    private DeleteRequest() {
        Retrofit retrofit = new Retrofit.Builder().baseUrl(URL.deleteRequest)
                .addConverterFactory(GsonConverterFactory.create()).build();
        HTTP_Method = retrofit .create (HTTP_Method .class);
    }

    public static synchronized DeleteRequest checkInstance() {
        if ( Retrofit == null)
            Retrofit  = new DeleteRequest();
